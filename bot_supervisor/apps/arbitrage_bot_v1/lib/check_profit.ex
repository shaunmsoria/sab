  ...
  def is_trade_profitable?(
    ...
    with router_address <-
      ...
      {:ok, direction, simulated_profit > 0, simulated_profit, simulated_profit_token_symbol,
       tradable_amount, gas_fee}
      |> IO.inspect(label: "sx1 is_trade_profitable? result")
    end
  end
  ...
  def simulate_profit_pre_gas(
    ...
    with tradable_amount <-
         ...
         {:ok, simulated_amount_out_reserve_0} <-
           router_address
           |> simulate_amount_output(tradable_amount, reserve1, reserve0)
           |> IO.inspect(label: "sx1 simulate_amount_output content"),
         {:ok, simulated_amount_out_reserve_1} <-
           router_address_searched
           |> simulate_amount_output(
             simulated_amount_out_reserve_0,
             reserve0_searched,
             reserve1_searched
           )
