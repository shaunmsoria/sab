...
describe("SABV1", () => {
  ...
  describe("executeTrade", function () {
    it("Uniswap to Sushiswap", async () => {
      ...
      const preTradeAmount = await ethers.provider.getBalance(account);

      console.log("sx1 ethers.parseUnits", ethers.parseUnits('1', 'ether'));
      console.log("sx1 getBalance wallet", preTradeAmount);

      let result;
      result = await sabv1.executeTrade(
        // token0, token1, sRouter, uRouter, 1);
        token0, token1, uRouter, sRouter, 100000000000);
      // token0, token1, sRouter, uRouter, ethers.parseUnits('1', 'ether'));
      // token0, token1, uRouter, sRouter, ethers.parseUnits('100', 'ether'));

      console.log("sx1 response from Balancer", result);

      expect(result.from).to.equal(account);

      const postTradeAmount = await ethers.provider.getBalance(account);
      console.log("sx1 postTradeAmount", postTradeAmount);
    })
  });
});