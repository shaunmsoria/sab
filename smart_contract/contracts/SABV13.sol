...
contract SABV13 is IFlashLoanRecipient {
    ...
    function receiveFlashLoan(
        ...
        _executeSwap(routerPath, tokenPath, flashAmount, 0);

        routerPath[0] = router1;
        routerPath[1] = router0;

        tokenPath[0] = token1;
        tokenPath[1] = token0;


        _executeSwap(routerPath, tokenPath, IERC20(token1).balanceOf(address(this)), flashAmount);
    ...
    function _executeSwap(
        ...
        IUniswapV2Router02 _Router = _routerPath[0];
        // uint256 _startAmountIn = IERC20(_tokenPath[0]).balanceOf(address(this));

        require(
            IERC20(_tokenPath[0]).approve(address(_Router), _amountIn),
